...
static const struct temp_info apple_soc_smc_temp_table[] = {
	...
	{SMC_KEY(TB0T),"Battery TS_MAX(TB0T)"},
	{SMC_KEY(TB1T),"Battery TS1(TB1T)"},
	{SMC_KEY(TB1T),"Battery TS2(TB2T)"},
	{SMC_KEY(TCHP),"CHARGER, BETWEEN INDUCTOR AND MOSFETS(TCHP)"},
	{SMC_KEY(TCMb),"TEMP (TCMb)"},
	{SMC_KEY(TCMz),"TEMP (TCMz)"},
	{SMC_KEY(TH0T),"LOWERLEFTCORNEROFNANDDEVICES"},
	{SMC_KEY(TH0x),"MaxNANDProximityTemp"},
	{SMC_KEY(TIOP),"I/OPROXIMITY"},
	{SMC_KEY(TIOP),"ThunderboltProximity"},
	{SMC_KEY(TMVR),"3.8VAONVRBETWEENPHASE2AND"},
	{SMC_KEY(TMVR),"P3V8AON_THMSNS"},
	{SMC_KEY(TPMP),"MASTERPMU,BETWEENBUCK0ANDBUCK1INDUCTORS"},
	{SMC_KEY(TPMP),"MPMU"},
	{SMC_KEY(TPSP),"SLAVEPMU,BUCK10INDUCTORPROXIMITY"},
	{SMC_KEY(TPSP),"SPMU"},
	{SMC_KEY(TSCD),"SOCCOREAREABACKSIDE(TOP)"},
	{SMC_KEY(TVS0),"P1V8VDDH_THMSNS1?"},
	{SMC_KEY(TVS1),"P1V8VDDH_THMSNS2?"},
	{SMC_KEY(TVSx),"P1V8VDDH_THMSNSxmax"},
	{SMC_KEY(TW0P),"AirportProximity"},
	{SMC_KEY(TW0P),"WLANBT,BACKSIDE"},
	{SMC_KEY(TW0P),"WLBTtemperature"},
	{SMC_KEY(Tc0b),"SOCAvgw/offsetClusterDieTemp"},
	{SMC_KEY(Th0x),"MaxNANDProximityTemp"},
	{SMC_KEY(Th1a),"GPUTemp"},
	{SMC_KEY(Th2a),"PCPUTemp"},
	{SMC_KEY(Ts1a),"GPUMTRDieTemp"},
	{SMC_KEY(Ts2a),"PCPUMTRDieTemp"},
